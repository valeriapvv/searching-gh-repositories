import {encodeForHtml} from '../utils.js';
...
const getRepoItemTemplate = (repoItem = {}) => {
  ...
  return (`
    <li class="main__item repo">
      <h3 class="repo__name">
        <a class="repo__link" href="${repoUrl}" target="_blank">${encodeForHtml(name)}</a>
      </h3>

      <dl class="repo__info">
        <div class="repo__content">
          <dt class="repo__caption">ID</dt>
          <dd class="repo__text">${encodeForHtml(id)}</dd>
        </div>

        <div class="repo__content">
          <dt class="repo__caption">Language</dt>
          <dd class="repo__text">${encodeForHtml(language ?? EMPTY_VALUE_STRING)}</dd>
        </div>
        ...
        <div class="repo__content repo__content--row">
          ...
          <dd class="repo__text">${encodeForHtml(description ?? EMPTY_VALUE_STRING)}</dd>
        </div>
      </dl>
    </li>
  `);
